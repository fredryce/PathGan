...
for id_remove in ids_to_remove:
    ...
    modelG = Sequential()
    modelG.add(Dense(int(TRIP_SIZE/4) * 1 * 128 , activation="relu", input_dim=latent_dim))
    modelG.add(Reshape((int(TRIP_SIZE/4), 1, 128)))
    modelG.add(UpSampling2D(size=(2,2)))
    modelG.add(Conv2DTranspose(128, kernel_size=(3,2), padding="same"))
    modelG.add(BatchNormalization(momentum=0.8))
    modelG.add(Activation("relu"))
    modelG.add(UpSampling2D(size=(2,2)))
    modelG.add(Conv2DTranspose(64, kernel_size=(3,2), padding="same"))
    modelG.add(BatchNormalization(momentum=0.8))
    modelG.add(Activation("relu"))
    modelG.add(Conv2DTranspose(32, kernel_size=(3,2), padding="same"))
